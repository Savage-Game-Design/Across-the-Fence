...
if (_is_unit_player) then
{
	...
	[[_unit],"hunger"] call vn_an_fnc_change_player_stat;
	[[_unit],"thirst"] call vn_an_fnc_change_player_stat;
	...
	if (_is_killer_player) then
	{
		...
		{
			_kill_type = "suicides";
			[crew _killer,_kill_type] call vn_an_fnc_change_player_stat;
		}
		else
		{
			// _unit is another player report as - friendlyfire
			_kill_type = "friendlyfire";
			[[_instigator],_kill_type] call vn_an_fnc_change_player_stat;
		};
	};
	...
	if (_is_killer_player) then
	{
		...
		if (vehicle _unit isKindOf "Man") then
		{
			...
			else
			{
				...
				[_units,"rank"] call vn_an_fnc_change_player_stat;
				[_units,_kill_type] call vn_an_fnc_change_player_stat;
