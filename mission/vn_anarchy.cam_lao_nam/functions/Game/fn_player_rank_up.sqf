/*
  ...
  Example Usage:
	[_rank] call vn_an_fnc_player_rank_up;
...
([player] call vn_an_fnc_unit_to_rank) params ["_icon", "_rank_text", "_pointsneeded"];

// check if rank has changed
if (_last_rank_text isEqualTo _rank_text) then
{
	// same rank
	"rank" cutText [format["<t font='VeteranTypewriter' color='#FFFFFF' align='center' size='2'>+ %1 %2</t>",_diff,localize "STR_vn_an_rankpoint_short"], "PLAIN DOWN", -1, true, true];
} else {
	// rank changed
	"rank" cutText [format["<img size='5' image='%1'/><t font='VeteranTypewriter' color='#FFD700' size='5'>%5</t><br/><t font='VeteranTypewriter' color='#FFFFFF' align='center' size='2'>+ %3 %4</t><t font='VeteranTypewriter' align='right'>Rank: %2</t>",_icon,_rank_text,_diff,localize "STR_vn_an_rankpoint_short",localize "STR_vn_an_levelup"], "PLAIN DOWN", -1, true, true];
	// store last rank
