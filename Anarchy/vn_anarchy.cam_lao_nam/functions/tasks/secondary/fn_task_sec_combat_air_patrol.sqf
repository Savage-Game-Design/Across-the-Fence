...
_dataStore setVariable ["move_to_area", {
	...
	} forEach units (missionNamespace getVariable ["GreenHornets", grpNull]); 
}];
...
_dataStore setVariable ["patrol_area", {
	...
	for "_i" from 1 to (1 - count _currentAircraft) do {
		...
		private _spawnPosition = _patrolPosition getPos [1000, ((getMarkerPos "respawn_west_greenhornets") getDir _patrolPosition) + random 45];
		private _aircraftClass = selectRandom selectRandom [vehicles_nva_planes, vehicles_nva_helis];
		private _spawnInfo = [_spawnPosition, _spawnPosition getDir _patrolPosition, _aircraftClass, east call vn_mf_fnc_create_group] call BIS_fnc_spawnVehicle;
		...
		private _firstWaypoint = _group addWaypoint [_patrolPosition, 50];
		_group addWaypoint [_patrolPosition getPos [10000, 340 + random 65], 50];
	...
	//Delete aircraft from the current aircraft list, and mark them for cleanup
	{
		...
		[_aircraft] call vn_mf_fnc_cleanup_addItems;
...
_dataStore setVariable ["FINISH", {
	...
	[_taskDataStore getVariable ["currentAircraft", []]] call vn_mf_fnc_cleanup_addItems;
