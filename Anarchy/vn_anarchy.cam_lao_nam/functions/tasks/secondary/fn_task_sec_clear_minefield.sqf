...
_taskDataStore setVariable ["INIT", {
	...
	for "_i" from 1 to _mineQuantity do {
		...
		if !(_minePosition isFlatEmpty [-1, -1, -1, 1, 0, false, objNull] isEqualTo []) then {
			...
			_mines pushBack ([_mineType, _minePosition, [], 0] call vn_mf_fnc_create_mine);
		};
	};
	...
	for "_i" from 1 to _unitCount do {
		...
		if (!surfaceIsWater _spawnPosition) then {
			private _group = [east] call vn_mf_fnc_create_group;
			_groups pushBack _group;
			_units pushBack ([_group, selectRandom units_vc_marksman, _spawnPosition, [], 10, "NONE"] call vn_mf_fnc_create_unit);
