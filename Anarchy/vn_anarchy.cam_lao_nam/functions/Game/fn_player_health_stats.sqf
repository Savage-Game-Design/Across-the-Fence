...
{
	private _unit = _x;
	private _hunger = _unit getVariable ["vn_mf_hunger",1];
	private _thirst = _unit getVariable ["vn_mf_thirst",1];
	private _attributes = _unit getVariable ["vn_mf_attributes",[]];
	...
	[_unit,"hunger",-(_hunger_loss_rate * _hunger_loss_factor)] call vn_mf_fnc_change_player_stat;
	[_unit,"thirst",-(_thirst_loss_rate * _thirst_loss_factor)] call vn_mf_fnc_change_player_stat;

	// force all players to save every 60 seconds to prevent roll back if server crashes
	_ticktime = diag_tickTime;
	_savetime = _unit getVariable ["vn_mf_savetime",0];
	if (_ticktime > _savetime) then
	{
		_unit setVariable ["vn_mf_savetime",_ticktime + 60];
