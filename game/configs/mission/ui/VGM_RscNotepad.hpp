class VGM_RscNotepad
{
    ...
    scale = QUOTE(1/VGM_NOTEPAD_SIZE_FACTOR);
    direction[] = {0, 0, 1};
    up[] = {0, 1, 0};

    x = QUOTE(safeZoneX + VGM_NOTEPAD_W/2);
    y = 0.55; z = 0.22;
    xBack = QUOTE(safeZoneX + VGM_NOTEPAD_W/8);
    yBack = 0.15; zBack = 1;
        ...
        class Main
        {
            ...
            class Controls
            {
                class Main: ctrlControlsGroup
                {
                    ...
                    x = QUOTE(1.75 * VGM_NOTEPAD_GRID_W);
                    y = QUOTE(10.5 * VGM_NOTEPAD_GRID_H);
                    w = QUOTE(VGM_NOTEPAD_W);
                    h = QUOTE(VGM_NOTEPAD_H);
