...
Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam erat, sed diam voluptua. At vero eos et accusam et justo duo dolore."; // Max length approx 199
        ...
        private _previousSkillCount = -1;
        {
            ...
            if (_forEachIndex > 0) then {
                ...
                private _wLineH = (_previousSkillCount max _currentSkillCount - 1);
                _wLineH = _wLineH * _wSkill + (_wLineH+1) * VGM_GRID_MARGIN * VGM_GRID_W;
                _ctrlSkillLineH ctrlSetPosition [
                    // Middle of tree, go back half the width of the line and a bit
                    0.5 * _wSkillTree - 0.5 * _wLineH - 0.25 * VGM_GRID_MARGIN * VGM_GRID_W,
                    _yPos,
                    _wLineH,
                    VGM_GRID_MARGIN * VGM_GRID_H
                ];
                _ctrlSkillLineH ctrlCommit 0;
                _previousSkillCount = _currentSkillCount;
            };
            _yPos = _yPos + 0.5 * VGM_GRID_H;
            // Center the controls
            ...
            _levelSkills apply {
                private _xLineV = _xPos + 0.5 * _wSkill - 0.5 * VGM_GRID_MARGIN * VGM_GRID_W;
                if (_forEachIndex > 0) then {
                ...
                _xPos = _xPos + _wSkill + VGM_GRID_MARGIN * VGM_GRID_W;
            };
        ...
        private _wRootLine = (_previousSkillCount - 1) * _wSkill + _previousSkillCount * VGM_GRID_MARGIN * VGM_GRID_W;
        _ctrlSkillLineHRoot ctrlSetPositionW _wRootLine;
        _ctrlSkillLineHRoot ctrlSetPosition [0.5 * _wSkillTree - 0.5 * _wRootLine, _yPos, _wRootLine, VGM_GRID_MARGIN * VGM_GRID_H];
        ...
        _ctrlSkillLineVRoot ctrlSetPosition [
            0.5 * _wSkillTree - 0.5 * VGM_GRID_MARGIN * VGM_GRID_W,
            _yPos
        ];
