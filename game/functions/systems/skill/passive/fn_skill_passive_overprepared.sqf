/*
    ...
    Example(s):
        true call vgm_c_fnc_skill_passive_overprepared
 */

params ["_known"];

if (!_known) exitWith {
    player removeEventHandler ["Fired", player getVariable "vgm_c_skill_passive_overpreparedEH"];
};
...
private _eh = player addEventHandler ["Fired", {
    ...
    private _usedInMission = _unit getVariable ["vgm_c_skill_passive_overpreparedMission", createHashMap];
    private _currentMission = [] call vgm_c_fnc_missions_getCurrentMission;
    if (_usedInMission isEqualTo _currentMission) exitWith {};
    _unit getVariable ["vgm_c_skill_passive_overpreparedMission", _currentMission];
...
player setVariable [ "vgm_c_skill_passive_overpreparedEH", _eh];
