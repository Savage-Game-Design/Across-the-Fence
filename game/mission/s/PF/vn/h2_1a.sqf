//Land_vn_hut_02
isNil{params["_b"];
if(isNil{_b getVariable"PF_B"})then{_b setVariable["PF_B","h2_1a"]};
...
_bag setPos(_b modelToWorld[-0.7,0,1.49]);
_bag setDir(_dir+20);

_box setPos(_b modelToWorld[1.64,-0.8,1.49]);
_box setDir _dir;

_chair setPos(_b modelToWorld[0.2,3.78,1.415]);
_chair setDir(_dir-5);

_desk setPos(_b modelToWorld[1.66,3.27,1.415]);
_desk setDir _dir;

_food setPos(_b modelToWorld[1.4,-1.55,1.49]);

_junk setPos(_b modelToWorld[0.2,2.4,1.51]);
_junk setDir _dir;

_pot setPos(_b modelToWorld[1.45,3.5,1.87]);
_pot setDir(_dir+145);

_rug setPos(_b modelToWorld[-0.8, 3.2, 1.4]);
_rug setDir(_dir+55);

_tea setPos(_b modelToWorld[1.6,3,1.87]);
