//Land_vn_hut_01
isNil{params["_b"];
if(isNil{_b getVariable"PF_B"})then{_b setVariable["PF_B","h1_1a"]};
_dir=getDir _b;
...
_cloth setPos(_b modelToWorld[0.65, 0.1, 5]);
_cloth setDir(_dir+53.921);

_food setPos(_b modelToWorld[2.36, 3.29, 1.3]);

_food2 setPos(_b modelToWorld[-0.3, -1.9, 1.973]);

_jug setPos(_b modelToWorld[1.75, 3.45, 1.3]);
_jug setDir(_dir+45);

_rugs setPos(_b modelToWorld[-0.55, 2.45, 1.32]);
_rugs setDir(_dir+90);

_rug setPos(_b modelToWorld[1.3, 1.5, 1.28]);
_rug setDir(_dir+105);

_rug2 setPos(_b modelToWorld[0, 0, 1.28]);
