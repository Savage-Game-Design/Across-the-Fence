...
private _duplicates = createHashMap;

private _noDuplicates = true;

for "_i" from 0 to ((lnbSize _ctrlKeybinds select 0) -1) do {
    private _keyText = _ctrlKeybinds lnbText [_i, 1];
    _duplicates set [_keyText, (_duplicates getOrDefault [_keyText, 0]) + 1];
};
...
for "_i" from 0 to ((lnbSize _ctrlKeybinds select 0) -1) do {
	...
	private _setColor = if (_duplicates getOrDefault [_keyText, 1] > 1) then {
		_noDuplicates = false;
		_doubleColor
	} else {
		_defaultColor
