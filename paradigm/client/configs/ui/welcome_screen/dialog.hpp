...
class para_WelcomeScreen {
    ...
    class ControlsBackground {
        class Background: para_Overlay_StructuredText_Background {
            x = QUOTE(para_WelcomeScreen_X);
            w = QUOTE(para_WelcomeScreen_WIDTH);
            y = QUOTE(UIH(1) + UIH((30 / 4)));
            h = QUOTE(UIH(24) - UIH((30 / 4)));
    ...
    class Controls {
        ...
        class Scrollable: para_RscControlsGroupNoScrollbarH {
            x = QUOTE(para_WelcomeScreen_X + UIW(0.5));
            w = QUOTE(para_WelcomeScreen_WIDTH - UIW(1));
            y = QUOTE(UIH(1) + UIH((30 / 4)) + UIH(0.5));
        ...
        class BtnLeft: RscButton {
            x = QUOTE(para_WelcomeScreen_X);
            w = QUOTE(para_WelcomeScreen_WIDTH / 2.0075);
...
class para_ChangelogScreen {
    ...
    class Controls {
        ...
        class Scrollable: para_RscControlsGroupNoScrollbarH {
            x = QUOTE(para_WelcomeScreen_X + UIW(0.5));
            w = QUOTE(para_WelcomeScreen_WIDTH - UIW(1));
            y = QUOTE(UIH(1) + UIH(0.5));
            h = QUOTE(UIH(25) - UIH(1) - UIH(1));
            class Controls {
                class Text: para_Overlay_StructuredText_Background {
                    idc = 30;
                    x = 0;
